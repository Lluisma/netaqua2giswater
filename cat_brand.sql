...
CREATE OR REPLACE VIEW CAT_BRAND AS 
  ...
  SELECT  DISTINCT VALVULA_ARQUETA_MARCA,
	  	    VALVULA_ARQUETA_MARCA,
		     	null
	FROM    NA_FIGARO.CAT2_T_VALV_ARQUETA
  WHERE   VALVULA_ARQUETA_MARCA IS NOT NULL
  UNION
  SELECT  DISTINCT VALVULA_ARQUETA_MARCA,
	  	  	VALVULA_ARQUETA_MARCA,
			    null
	FROM    NA_LLISSADEVALL.CAT2_T_VALV_ARQUETA
  WHERE   VALVULA_ARQUETA_MARCA IS NOT NULL
  ...
  SELECT  DISTINCT BOCAREG_TIPUS_MARCA,
	 	      BOCAREG_TIPUS_MARCA,
			    null
	FROM    NA_FIGARO.CAT2_T_BREG_TIPUS
  WHERE   BOCAREG_TIPUS_MARCA IS NOT NULL
  UNION
  SELECT  DISTINCT BOCAREG_TIPUS_MARCA,
			    BOCAREG_TIPUS_MARCA,
			    null
	FROM    NA_LLISSADEVALL.CAT2_T_BREG_TIPUS
  WHERE   BOCAREG_TIPUS_MARCA IS NOT NULL
  ...
  SELECT  DISTINCT BOCAINC_TIPUS_MARCA,
		      BOCAINC_TIPUS_MARCA,
			    null
	FROM    NA_FIGARO.CAT2_T_BINC_TIPUS
  WHERE   BOCAINC_TIPUS_MARCA IS NOT NULL
  UNION
  SELECT  DISTINCT BOCAINC_TIPUS_MARCA,
			    BOCAINC_TIPUS_MARCA,
			    null
	FROM    NA_LLISSADEVALL.CAT2_T_BINC_TIPUS
  WHERE   BOCAINC_TIPUS_MARCA IS NOT NULL
  ...
  SELECT  DISTINCT VENTOSA_TIPUS_MARCA,
		      VENTOSA_TIPUS_MARCA,
			    null
	FROM    NA_FIGARO.CAT2_T_VENT_TIPUS
  WHERE   VENTOSA_TIPUS_MARCA IS NOT NULL
  UNION
  SELECT  DISTINCT VENTOSA_TIPUS_MARCA,
			    VENTOSA_TIPUS_MARCA,
			    null
	FROM    NA_LLISSADEVALL.CAT2_T_VENT_TIPUS
  WHERE   VENTOSA_TIPUS_MARCA IS NOT NULL
