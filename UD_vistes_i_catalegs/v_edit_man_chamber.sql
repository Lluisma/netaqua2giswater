...
CREATE OR REPLACE FORCE VIEW v_edit_man_chamber AS
  ...
  SELECT 
    ...
    SET_NODE_CAT( POU_MAT, POU_DIM, POU_SEC )    AS NODECAT_ID,
    1                                            AS SECTOR_ID,
    ...
    NULL                                         AS pol_id,
    NULL                                         AS "name",
    CAST(null AS NUMBER)                         AS max_volume,
    CAST(null AS NUMBER)                         AS util_volume,
    CAST(null AS NUMBER)                         AS "length",
    CAST(null AS NUMBER)                         AS width,
    CASE T1.EMBOC
      WHEN 1 THEN 'true'
      ELSE 'false'
    END                                          AS inlet,
    CASE T1.SOLERA_CANAL
      WHEN 1 THEN 'true'
      ELSE 'false'
    END                                          AS bottom_channel,
    CASE T1.POU_PRA
      WHEN 'Sí' THEN 'SI'
      WHEN 'No' THEN 'NO'
      ELSE NULL
    END                                          AS accessibility,
    T1.SORRER_ALT                                AS sander_depth
  FROM NS_MATARO.CL_V_NODE T1
        LEFT JOIN NS_MATARO.CL3_T_NODE T2 ON T1.ID_NODE = T2.ID_NODE
  WHERE T1.ESTAT IN ('A','B','X')
    AND T1.NODE_FUN IN ('B', 'C', 'S');
