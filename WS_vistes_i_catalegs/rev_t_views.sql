...
CREATE OR REPLACE VIEW REV_T_WJOIN AS
  ...
  UNION
  SELECT ID_ESCO CONNEC_ID, TO_NUMBER(CODI_COM) CODE, 2 EXPL_ID, DDM DATA_REV 
  FROM   NA_FIGARO.NA_V_ESCO 
  WHERE  DDM IS NOT NULL
  UNION
  SELECT ID_ESCO CONNEC_ID, TO_NUMBER(CODI_COM) CODE, 3 EXPL_ID, DDM DATA_REV 
...
CREATE OR REPLACE VIEW REV_T_VALVE AS
  ...
  SELECT ID_VALV NODE_ID, ID_VALV CODE,
         CASE         
           WHEN FUNCIO = 'REGULADORA' THEN 'VREGULADORA'
           ELSE 'VALVULA'
         END NODETYPE_ID, 2 EXPL_ID, INCID_OC, INCID_OM, DATA_DR, DATA_FR, REVI_ESTAT
  FROM   NA_FIGARO.NA_V_VALV
  WHERE  INCID_OC IS NOT NULL OR INCID_OM IS NOT NULL OR DATA_DR IS NOT NULL OR DATA_FR IS NOT NULL OR REVI_ESTAT IS NOT NULL 
  UNION
  SELECT ID_VDES NODE_ID, ID_VDES CODE, 'VDESCARREGA' NODETYPE_ID, 2 EXPL_ID, INCID_OC, INCID_OM, DATA_DR, DATA_FR, REVI_ESTAT
