...
CREATE OR REPLACE VIEW GW_MIGRA_NETAQUA.TMP_CAT_ELEMENT_ARQFONT AS
...
CREATE OR REPLACE VIEW GW_MIGRA_NETAQUA.TMP_CAT_ELEMENT_ARQUETA AS
	SELECT DISTINCT 
	       ID_VALV_ARQUETA                  id,
...
CREATE OR REPLACE VIEW GW_MIGRA_NETAQUA.TMP_CAT_ELEMENT_AIXETA AS
...
CREATE OR REPLACE VIEW GW_MIGRA_NETAQUA.CAT_ELEMENT AS
    ...
    UNION
    SELECT * FROM TMP_CAT_ELEMENT_OPE
    UNION 
    SELECT 'PLA_XX', 'PLACA', null, 'Placa', 'Placa - Altres / Desconegut', null, null, null, null, null, 'true' FROM DUAL
  	ORDER BY 1;
